...
public class AzureBlobStorageProvider extends AbstractStorageProvider {
    ...
    protected List<StorageObjectRef> doListObjects(String prefix, int maxKeys) {
        return client.listObjects(properties.getAzure().getContainer(), prefix, maxKeys);
    }

    @Override
    protected StorageHealth doHealth() {
        return client.ping() ? StorageHealth.up() : StorageHealth.degraded("Azure container is unavailable");
    }
