...
public class S3StorageProvider extends AbstractStorageProvider {
    ...
    private final S3StorageClient client;
    ...
    protected List<StorageObjectRef> doListObjects(String prefix, int maxKeys) {
        return client.listObjects(properties.getS3().getBucket(), prefix, maxKeys);
    }

    @Override
    protected StorageHealth doHealth() {
        return client.ping() ? StorageHealth.up() : StorageHealth.degraded("S3 bucket is unavailable");
    }
