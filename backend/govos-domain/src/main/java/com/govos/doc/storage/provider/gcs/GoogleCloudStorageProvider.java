...
public class GoogleCloudStorageProvider extends AbstractStorageProvider {
    ...
    protected List<StorageObjectRef> doListObjects(String prefix, int maxKeys) {
        return client.listObjects(properties.getGoogle().getBucket(), prefix, maxKeys);
    }

    @Override
    protected StorageHealth doHealth() {
        return client.ping() ? StorageHealth.up() : StorageHealth.degraded("GCS bucket is unavailable");
    }
