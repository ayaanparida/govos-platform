...
class AzureBlobStorageProviderTest {
    ...
    @Mock private AzureBlobStorageClient client;
    ...
    void setUp() {
        ...
        provider = new AzureBlobStorageProvider(client, properties, StorageTestFixtures.disabledMetrics(properties));
    }

    @Test
    void shouldDelegateStoreToClient() {
        StorageStoreRequest request = new StorageStoreRequest(
                UUID.randomUUID(), "container", "key", "text/plain", 4L, null);
        StorageStoreResult expected = new StorageStoreResult(new StorageObjectRef("container", "key"), null, 4L);
